...
module.exports = new function() {
            ...
            for (var i = 0; i < tagsArray.length; i++) {
                ...
                .then(function(tagConcepts){
                    for (var j = 0; j < tagConcepts.body.results.length; j++) {
                        console.log(tagConcepts.body.results[j]["domain:canonicalName"].toLowerCase() , tagsArray[tagConcepts.i].toLowerCase());
                        if (tagConcepts.body.results[j]["domain:canonicalName"].toLowerCase() === tagsArray[tagConcepts.i].toLowerCase()) {
                            console.log("matched tag concept", tagConcepts.body.results[j])
                            return {
                                    "i" : tagConcepts.i,
                                    "guid" : self._getguid(tagConcepts.body.results[j]["@id"])
                                }
                        } else {
                            console.log("didnt matc");
                            return {
                                    "i" : tagConcepts.i,
                                    "guid" : ""
                                }
                ...
                .then(function(results){
                    ...
                    console.log("GOT HERE", results.i);//, JSON.stringify(results));
                    ...
                    if (count == tagsArray.length) {
                        ...
                        //console.log("I AM HOPEFULLY  FINISHING BY BOW", storiesArray);
    ...
    this._getguid = function(fullUrl) {
        console.log('in get guid', fullUrl);
    ...
    this.getCworksFromGuid = function(guid, i) {
        ...
        //http://data.bbc.co.uk/ldp/creative-works-v2?api_key=rrnbeTUoajuHhTyUGE04msdolErjjrhm&about=3afab96b-01da-496b-a8b4-d0aba98d26ab&format=VideoFormat
            if (cachedValue == undefined) {
                console.log("getting cworks for ", guid, 'http://data.bbc.co.uk/ldp/creative-works-v2?api_key=rrnbeTUoajuHhTyUGE04msdolErjjrhm&about=' + guid);
                var options = {
    ...
    this.getAssetsFromCandy = function(assetObject) {
        console.log("ATTWEMTPTING CANDY");
        ...
        if (typeof(assetObject.cworks.results) !== 'undefined' && assetObject.cworks.results.length > 0) {
            console.log("and the cworks are cool");
            if (cachedValue == undefined) {
                var deferred = Q.defer();
                var self = this;
    ...
    this._correllate = function(storiesInGroups) {
        ...
        for (var i = 0; i < storiesInGroups.length; i++) {
            console.log("IN THIS FOR LOOPPPPPPP==================================");
            console.log(storiesInGroups.length, typeof storiesInGroups[i]);
            if (typeof storiesInGroups[i+1] !== 'undefined') {
